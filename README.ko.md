...
- **Display**: 멀티 CLI 세션 모니터 — 컨텍스트 구성 파이 차트, 접속 방식 배지(SSH/tmux/tailscale/mosh), 프로세스 생존 감지
- **History**: 프록시 레벨 대화 기록 — delta/full 저장, compaction 감지, 웹 리플레이 뷰어
- **Composition**: 실시간 컨텍스트 윈도우 분석 — 6개 카테고리 분류(user/assistant/tool_use/tool_result/thinking/system) + SNR 지표 + 중복 Read 추적
- **TUI**: `llm-relay top` — btop 스타일 터미널 모니터 (SSH 환경에서 브라우저 없이 사용 가능)
- **i18n**: 브라우저 locale 자동 감지 (en/ko), 서버사이드 `LLM_RELAY_LANG` 환경변수로 오버라이드
...
llm-relay top               # 실시간 터미널 모니터 (btop 스타일 TUI)
```
...
- `/display/` — 턴 카운터 + 컨텍스트 구성 파이 차트 + 접속 방식 배지 + CC/Codex/Gemini 세션 카드
- `/history/` — 세션 대화 리플레이 + compaction 타임라인
