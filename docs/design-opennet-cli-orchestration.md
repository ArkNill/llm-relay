...
> 2026-04-07 설계 확정. llm-relay 오픈망 확장 레이어.
> 관련 메모리: `project_opennet_cli_orchestration.md`, `project_mirror_strategy.md`

## Context

Mirror Agent는 폐쇄망(DGX + 로컬 LLM) 기반 설계. 대부분의 개발자는 오픈망 구독 서비스(CC Pro, ChatGPT Pro, Gemini Pro) 사용.
이 갭을 메우기 위해 llm-relay에 오픈망 확장 레이어를 추가하여, 월정액 구독자가 3개 CLI를 통합 관리할 수 있게 함.
...
| 2 | MA + CLI | Mirror Agent | Gemini/OpenAI API (user choice) | 3 CLIs = MCP tools | Yes |
| 3 | MA + DGX + CLI | Mirror Agent | DGX vLLM | 3 CLIs = delegation | Yes |
...
Adds table to existing proxy DB (`~/.cc-relay/usage.db`):
...
### Scenario 3: MA + DGX + CLI

Same as scenario 2. DGX vLLM = main inference, CLIs = auxiliary/delegation.
...
1. **Single DB file**: orchestration tables in existing `~/.cc-relay/usage.db`
...
9. **ZBook direct** (not Docker): CLI binaries + OAuth are on host, orch/mcp/api run direct. Proxy stays in Docker (8082)
...
| MA _get_tiers() hardcoded | DGX+Desktop fixed, ignoring inference_provider.py config | Refactored to config-based dynamic generation |
...
Scenario 2 blocker: `inference.py:_get_tiers()` hardcoded DGX+Desktop tiers, ignoring `inference_provider.py` config.
